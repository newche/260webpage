...
Cardiovascular diseases (CVDs) are the 1st cause of death globally. Heart failure is a common event caused by CVDs. Early detection and continuous health management are recommended for people who have high risk for CVDs. With the data set that contains 11 explanatory covariates that can be used to predict heart failure, our study aims to study how the important factors are related to heart attach event and to make prediction upon the given factors. 


### 2. Dataset


To be more detailed, the data set was created by combining 5 heart data sets. The combined final version data set contains 11 explanatory factors that are related to heart failure. The factors include basic information such like Age, Sex; physical exam results such like Chest pain type, resting blood pressure, cholesterol, fasting blood sugar, resting ECG, max heart rate, exercise-induced angina, oldpeak of ST, ST slope; and binary response variable heart disease. 

To clean the data, entries that with cholesterol being 0 are removed. Entries that with negative Oldpeak and 0 blood pressure are also removed. After cleaning, there are 745 rows in the data set and 11 explanatory factors and 1 response variable related to heart failure.
...
K-Nearest Neighbor model is fitted with all the 11 explanatory covariates. The best number of neighbors K will be selected by using 10-fold cross validation inside the training set again. K with highest accuracy will be selected as the best number of neighbors and used in the kNN model prediction.
...
Random Forest model is also performed. The best number of variable to random sample as candidates at each split is selected by Cross-validation within the training set created earlier. Number of trees are used as the default 150.
...
To examine the error rate versus the number of trees, the result is shown in *Figure 3. Error Rate vs. Number of Trees in Random Forest Model*. There is little change in error rate after about 150 trees. This makes the decision to use default 150 trees in random forest model valid.



The confusion matrix of random forest is shown in *Table 3. Confusion Matrix of Best Model - Random Forest*. There are only 2 samples are false negative and 1 sample is false positive, which yields very low error rate.
...
In terms of model prediction performance, with probability cutoff being 0.05, random forest has the best performance with test error rate 0.9%. It was followed by Logistic with Lasso selected variables (test error=0.127), Logistic with all variables (0.130), and lastly KNN with K=8 (0.127).



To avoid heart disease, the models suggest that you should monitor how the heart and blood flow reacts to exercise frequently, try to exercise more and avoid type II diabetes, push max heart rate to higher.
...
We assume the analysis is mostly successful, because we compare models by several measurement (error rate, sensitivity, specificity) and we also come up with the best hyper-parameters by cross-validation. The conclusion comes from a combination of the results from the models.



If there is more time, firstly we can fit more models, like PCA, boosting, etc. All the analysis can be conducted with more details. More variables can be incorporated if possible. 
